namespace work3{
    int work(const vector<string> & clientid, const vector<string> & serverid, 
            ...
            const int n, const int m, const int T, const int qos_constraint, const double W = 1.05, const int seed = 131131){
        
        vector<double> sdemand(m);
        vector<int> deg(m);
        for (int t = 0; t < T; t++)
            for (int i = 0; i < m; i++)
                sdemand[i] += demand[t][i];
        for (int i = 0; i < n; i++)
            for (int j = 0; j < m; j++) if (qos[i][j] < qos_constraint) deg[j]++;
        for (int i = 0; i < m; i++) sdemand[i] = sdemand[i] / deg[i] / T;
        vector<double> out(n);
        for (int i = 0; i < n; i++)
